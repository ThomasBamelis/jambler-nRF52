...
impl JammerState for CalibrateIntervalTimer {
    ...
    fn config(&mut self, parameters: &mut StateParameters<impl JamBLErHal>) -> Result<(), StateError> {
        if let Some(config) = &parameters.config {
            self.interval = config.interval.ok_or(StateError::InvalidConfig("Interval not provided for calibrating interval timer."))?;
            Ok(())
        }
        else {
            Err(StateError::InvalidConfig("Config not provided for calibrating interval timer."))
        }
    }
    ...
    fn initialise(
        ...
        // Should not require an interval timer, None should not change anything (timer gets reset anyway on a state transition).// set interval
    ...
    fn handle_interval_timer_interrupt(
        ...
        match self.point_in_sequence.clone() {
            ...
            CalibrationSequence::PeriodicToPeriodic => {
                ...
                // Ask for countdown
                ret.timing_requirements = Some(IntervalTimerRequirements::Countdown(self.interval));
            }
            CalibrationSequence::PeriodicToCountdown => {
                ...
                let state_change_to_first_interrupt_time = self.periodic_after_state_change_time - self.state_change_start_time;
                let periodic_no_change_time = self.periodic_after_periodic_time - self.periodic_after_state_change_time;
                let interval_timer_change_time = self.countdown_after_periodic - self.periodic_after_periodic_time;
                ...
                ret.state_message = Some(StateMessage::IntervalTimerDelays(state_change_to_first_interrupt_delay, periodic_no_change_delay, interval_timer_change_delay));
                ...
                ret.state_transition = Some((JamBLErState::Idle, None));
            }

        }
        // Always give back some for realism
        Ok(Some(ret))
    }
    ...
    fn is_valid_transition_from(&mut self, old_state: &JamBLErState) -> Result<(), StateError> {
        match old_state {
            ...
            _ => Err(StateError::InvalidStateTransition(
                "Calibrate interval timer from a non-idle state.",
            )),
        }
    }
    ...
    fn is_valid_transition_to(&mut self, new_state: &JamBLErState) -> Result<(), StateError> {
        match new_state {
            Idle => Ok(()),
            _ => Err(StateError::InvalidStateTransition(
                "Calibrate interval timer to a non-idle state.",
            )),
        }
    }
}
