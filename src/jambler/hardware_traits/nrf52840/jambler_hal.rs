...
impl JamBLErHal for Nrf52840JamBLEr {
    ...
    fn config_discover_access_addresses(&mut self, phy: BlePHY, channel: u8) {
        ...
        if self.radio_peripheral.power.read().power().is_disabled() {
            panic!("Power disabled while harvest packet config change");
        }
        if !self.radio_peripheral.state.read().state().is_disabled() {
            panic!("Radio not disabled while harvest packet config change");
        }

        // Should only listen on general purpose channels. Remember u8 is unsigned.
        if channel > 36 {
            panic!(
                "Invalid channel for discovering access addresses: {}",
                channel
            );
        }
    ...
    fn read_discovered_access_address(&mut self) -> Option<(u32, i8)> {
        ...
        match self.current_phy.unwrap() {
            BlePHY::Uncoded1M => {
                ...
                for _ in 0..3 {
                    let (first_header_byte, second_header_byte) = dewithen_16_bit_pdu_header(
                        received_bytes[4],
                        received_bytes[5],
                        self.current_channel.unwrap(),
                    );

                    aa = (received_bytes[3] as u32) << 24
                        | (received_bytes[2] as u32) << 16
                        | (received_bytes[1] as u32) << 8
                        ...
                        && is_valid_aa(aa, BlePHY::Uncoded1M)
                    {
                        //TODO delete
                        rprintln!(
                            "Considered packet: AA {:#010x} with rssi {} header {:#010b} {:#010b}",
                            aa,
                            rssi,
                            first_header_byte,
                            second_header_byte
    ...
    fn idle(&mut self) {
        // Disable all interrupts
        self.radio_peripheral.intenclr.write(|w| {
            w.ready()
                .clear()
                .address()
                .clear()
    ...
    fn config_harvest_packets(
        ...
        if radio.power.read().power().is_disabled() {
            panic!("ERROR: power disabled while harvest packet config change");
        }
        if !radio.state.read().state().is_disabled() {
            panic!("ERROR: radio not disabled while harvest packet config change");
        }
        ...
        radio
            .base0
            .write(|w| unsafe { w.bits(access_address << 8) });
        radio
            .prefix0
        ...
        match crc_init {
            Some(crc_init) => {
                radio
                    .crcinit
                    .write(|w| unsafe { w.crcinit().bits(crc_init) });
            }
            None => {
                radio.crcinit.reset();
            }
    ...
    fn handle_harvest_packets_radio_interrupt(&mut self) -> Option<HalHarvestedPacket> {
        ...
        rprintln!("First header byte: {:08b}", unsafe {
            core::ptr::read_volatile(&self.receive_buffer[0])
        });
        ...
        if cp_bit_set {
            ...
            let misplaced_crc_byte: u8 = unsafe {
                core::ptr::read_volatile(&self.receive_buffer[(1 + payload_len + 1) as usize])
            };
        ...
        let reverse_calculated_crc_init =
            reverse_calculate_crc_init(received_crc, &self.receive_buffer, pdu_length);
    ...
    fn harvest_packets_busy_wait_slave_response(
        ...
        self.radio_peripheral
            .packetptr
            .write(|w| unsafe { w.packetptr().bits(slave_pdu_buffer.as_ptr() as u32) });
        //rprintln!("Packetpointer set in busy wait: 0x{:08X}", slave_pdu_buffer.as_ptr() as u32);
        ...
        match slave_phy {
            ...
            BlePHY::CodedS2 => {
                ...
                self.radio_peripheral.pcnf0.modify(|_, w| unsafe {
                    w.plen().long_range().cilen().bits(2).termlen().bits(3)
                });
            }
            BlePHY::CodedS8 => {
        ...
        if master_cp_bit_set {
            ...
            // This is an index (start from 0)
            let misplaced_crc_byte: u8 = unsafe {
                core::ptr::read_volatile(&master_pdu_buffer[(1 + payload_len + 1) as usize])
            };
            // Get malformed crc the radio thaught it was due to misalignment
            let malformed_rxcrc = self.radio_peripheral.rxcrc.read().bits();
            // Reconstruct the actual crc
            master_received_crc =
                (reverse_bits(misplaced_crc_byte) as u32) << 16 | (malformed_rxcrc >> 8);
        }
        ...
        // TODO DOES NOT WORK ON RELEASE BUILD, THE BUSY WAIT GETS OPTIMISED AWAY
        ...
        while !self
            ...
            .events_address()
            .bits()
            && busy_waiter < microsec_wait_approx
        {
        ...
        */
            ...
            .bits()
        {
            ...
            if slave_cp_bit_set {
                ...
                // This is an index (start from 0)
                let misplaced_crc_byte: u8 = unsafe {
                    core::ptr::read_volatile(&slave_pdu_buffer[(1 + payload_len + 1) as usize])
                };
                // Get malformed crc the radio thaught it was due to misalignment
                let malformed_rxcrc = self.radio_peripheral.rxcrc.read().bits();
                // Reconstruct the actual crc
                slave_received_crc =
                    Some((reverse_bits(misplaced_crc_byte) as u32) << 16 | (malformed_rxcrc >> 8));
            }
        }
        ...
        match slave_received_crc {
            ...
            Some(slave_crc) => Some((
                (master_received_crc, master_rssi),
...
fn is_valid_discover_header(first_byte: u8, second_byte: u8) -> bool {
    let ret = (((first_byte & 0b1100_1000) == 0b1000_0000)
        || ((first_byte & 0b0001_0011) == 0b0000_0001))
        && second_byte == 0;
    true
}
