...
impl SerialController {
    ...
    pub fn new(device : hal::pac::UARTE1, mut pins_ctrl : hal::uarte::Pins) -> SerialController {
        ...
        device.psel.cts.write(|w| {
                w.connect().disconnected()
        });
        device.psel.rts.write(|w| {
            w.connect().disconnected()
        });
        ...
        device
            .config
            .write(|w| w.parity().variant(hal::pac::uarte0::config::PARITY_A::EXCLUDED));

        // Configure frequency to 9600 baud.
        device.baudrate.write(|w| w.baudrate().variant(hal::pac::uarte0::baudrate::BAUDRATE_A::BAUD9600));
        ...
        device.intenset.write(|w| 
            w.endrx().variant(hal::pac::uarte0::intenset::ENDRX_AW::SET)
        );
    ...
    pub fn start_listening(&mut self) -> () {
        ...
        self.uarte1_peripheral.rxd.ptr.write(|w|
            unsafe { w.ptr().bits(self.rx_byte.as_ptr() as u32) });


        // we only read in one byte in our 1 byte long buffer
        self.uarte1_peripheral.rxd.maxcnt.write(|w|
            unsafe { w.maxcnt().bits(1 as _) });
        ...
        // Assume communication is ascii
            ...
            if new_char == self.splitter {
                ...
                match self.received_string.push(new_char) {
                    Err(_) => {
                        rprintln!("Receive buffer full, dropped {} and flushed buffer.", new_char);
                        Some(self.stop_receiving())
                    }
                    _ => {None}
                }
    ...
    pub fn send_string(&mut self, s : String<U256>) -> () {
        ...
        if bytes.len() + self.send_buffer.len() as usize > self.send_buffer.capacity() as usize {
            ...
            rprintln!("Output buffer overflow. Ommitting string: {}", String::from_utf8(bytes).unwrap());
        }
        else {
            for byte in bytes {
    ...
    fn sending_string(&mut self) -> () {
        ...
        match self.send_buffer.dequeue() {
                Some(byte) => {
                    ...
                    if !self.sending {
                        self.uarte1_peripheral.intenset.write(|w| 
                            w.endtx().variant(hal::pac::uarte0::intenset::ENDTX_AW::SET)
                        );
                    ...
                    self.uarte1_peripheral.txd.ptr.write(|w| 
                        unsafe { w.ptr().bits(self.tx_byte.as_ptr() as u32) }
                    );

                    // I will take the easy way first and send byte per byte.
                    self.uarte1_peripheral.txd.maxcnt.write(|w|unsafe { w.maxcnt().bits(1 as _) });
                ...
                None => {
                    ...
                    self.uarte1_peripheral.intenclr.write(|w| 
                        w.endtx().variant(hal::pac::uarte0::intenclr::ENDTX_AW::CLEAR)
                    );
