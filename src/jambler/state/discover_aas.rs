...
impl JammerState for DiscoverAas {
    ...
    fn config(&mut self, parameters: StateConfig) -> Result<(), StateError> {
        if let Some(phy) = parameters.phy {
            ...
            Err(StateError::InvalidConfig(
                "PHY not provided for discovering AAs",
            ))
        }
    }
    ...
    fn handle_interval_timer_interrupt(
        ...
        rprintln!(
            "Sniffing AAs on new channel {}. Timestamp: d {} h {} min {} s {} ms {}",
            self.channel,
            (instant_in_microseconds / (24 * 60 * 60 * 1000_000)),
            (instant_in_microseconds / (60 * 60 * 1000_000)) % 24,
            (instant_in_microseconds / (60 * 1000_000)) % 60,
            (instant_in_microseconds / 1_000_000) % 60,
            (instant_in_microseconds / 1000) % 1000
        );
